...
**Gating:** `IsGateEligible` limits which rule families can contribute to exit-code failure in `developer` mode. Eligibility is declared alongside confidence in the single `ruleFamilies` table in `internal/model/model.go`; the wedge families are `SCM-`, `CI-PRT-`, `CI-EXFIL-`, `CI-ABUSE-`, `CI-SECRET-`, `POL-GHA-`, `AGT-*` skill/memory/trust surfaces, `GRAPH-`, `CLOUD-ID-`, `DISC-MCP-`, and `DOM-TYPO-`. A definitive family must either gate or record an `UngatedReason`, which is enforced by test — the two properties used to live in separate slices, and the drift between them let HIGH/definitive findings pass a `--fail-on high` run. In `ir` and `deep`, no rule family is gate-eligible—`--fail-on` still runs but only gate-eligible findings count toward threshold, so these modes stay advisory unless you switch mode or change eligibility semantics later.

When a run passes but findings at or above `--fail-on` were excluded by eligibility, the CLI emits a `WARN` naming the rule IDs, so a silent fail-open is never indistinguishable from a clean scan.
