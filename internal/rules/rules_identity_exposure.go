	...
	return []Rule{
		{
			...
			Target:         TargetContent,
			PathPattern:    regexp.MustCompile(`(?i)(^|/)(iam|rbac|polic(y|ies)|roles?|clusterroles?|rolebindings?|serviceaccounts?)(/|[._-]|$)|\.(tf|rego)$`),
			ContextPattern: regexp.MustCompile(`(?i)(Effect|Action|Resource|Principal|Statement|Policy|Role|ClusterRole|RoleBinding|ServiceAccount|apiVersion|kind|rules)`),
			ContextWindow:  5,
