...
- **`ruleFamilies`** (`internal/model/model.go`) — the single source of truth for rule-family classification. Each entry declares a prefix, its default `ConfidenceClass`, whether it gates in developer mode, and, for definitive families that do not gate, an `UngatedReason`. **Add new rule families here.** Tests enforce that every declared prefix is actually emitted by some rule or check, and that definitive families gate or explain why they do not.
...
- **`GateSuppressedRuleIDs`** — rule IDs that met the severity threshold but were excluded by gate eligibility. The CLI warns with these on an otherwise-passing run so fail-open is never silent.
